...
export class NewExhibitionComponent implements OnInit {
  ...
  validation_message = {
    ...
    'exhibitionDescription' : [
      { type: 'required', message: 'Please provide a Description for the Exhibition'},
      { type: 'minlength', message: 'Must be more than 2 characters'},
    ...
    'exhibitionTypeID' : [
      { type: 'min', message: 'Please select a type'},
    ],
    'organisationID' : [
      { type: 'required', message: 'Please select an organisation'},
    ],
    'venueID' : [
      { type: 'required', message: 'Please select an Venue'},
    ]
  }

  constructor(private route: Router, private modalService: NgbModal, private toastr: ToastrService, public exhibitionService: ExhibitionService, private formBuilder: FormBuilder) {
  ...
  ngOnInit(): void {
    ...
    this.exhibitionForm = this.formBuilder.group({
      exhibitionID: [0],
      exhibitionName: ['', Validators.compose([
        Validators.required, 
        Validators.minLength(2),
        Validators.maxLength(50)
      ])],
      exhibitionDescription: ['', Validators.compose([
        Validators.required, 
        Validators.minLength(2),
      ...
      // scheduleID: [0],
      organisationID: [1, Validators.min(1)],
  ...
  onSubmit(event){
    ...
    this.exhibitionService.postExhibition(this.exhibitionForm.value).subscribe(res => {
