...
export class AddArtworkComponent implements OnInit {
  ...
  error_messages = {
    ...
    'SurfaceTypeID': [
      { type: 'required', message: 'Surface Typeis required.' }
    ],

    'MediumTypeID': [
      { type: 'required', message: 'Medium Type is required.' }
    ],
  ...
  ngOnInit(): void {
    ...
    this.data.getAllSurfaceType().then((result) => { console.log(result); this.ListSurfaceTypes = result });
    this.data.getAllMediumTypes().then((result) => { console.log(result); this.ListMediumTypes = result });
    this.data.getAllArtworkStatuses().then((result) => { console.log(result); this.ListArtworkStatuses = result });
    this.data.getAllDimesisons().then((result) => { console.log(result); this.ListArtworkDimensions = result });
    this.data.getAllFrameColors().then((result) => { console.log(result); this.ListFrameColours = result });
    this.data.getAllArtworkTypes().then((result) => { console.log(result); this.ListArtworkTypes = result });
...
removeImage() {
  ...
  onSubmit(event,addArtworkModal) {
    ...
    this.addArtworkForm.get('ArtworkPicture1BASE64').setValue(this.picture1base64) ;
    ...
    if (this.addArtworkForm.invalid) {
      ...
      this.data.addArtwork(this.addArtworkForm.value).then(success => {
